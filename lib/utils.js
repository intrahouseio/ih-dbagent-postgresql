...
function formValues(data, columns, tdate) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < columns.length; j++) {
      ...
      //если в запросе число, то записываем как число иначе string
...
function getQueryStrId(query, idarr) {
  ...
  const to = query.end;
  let res = `select * from records ${formWhereQueryId(idarr, from, to)} order by ts`;
  return res;
}
...
function getQueryStrDn(query, dnarr) {
  ...
  const from = query.start;
  const to = query.end;
  let res =`select * from timeline ${formWhereQueryDn(dnarr, from, to, 'start', '"end"')} order by start`
  return res;
}

function formWhereQueryDn(dnarr, from, to, ts_start_name = 'ts', ts_end_name = 'ts') {
  let query = '';
  let first = true;
...
function formWhereQueryId(idarr, from, to, ts_start_name = 'ts', ts_end_name = 'ts') {
  let query = '';
  let first = true;
