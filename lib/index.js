...
module.exports = async function(channel, opt, logger) {
  ...
  try {
    ...
    await client.query(`SELECT create_hypertable('records','ts', 
        ...
        if_not_exists => TRUE);`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_records_dn ON records (dn, ts DESC);`);
   ...
   */
    ...
    async function getDBSize() {
      const sqlQuery = `SELECT pg_size_pretty( pg_total_relation_size('records') );`; 
      ...
      let fileSize = parseInt(dbSizeArr[0].pg_size_pretty)/1024;
