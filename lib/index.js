...
module.exports = async function (channel, opt, logger) {
  ...
  try {
    ...
    channel.on('message', ({ id, type, query, payload, table }) => {
      ...
      if (type == 'update') return updateCustom(query, payload, table);
      ...
      if (type == 'updateJson') return updateJson(query, payload, table); //[{id, $set:{field1:’newval’}}]
 ...
 */
    ...
    if (query.sql) {
      ...
      for (j = 0; j < payload.length; j++) {
        ...
        for (i = 0; i < arr.length; i++) {
          const value = `JSONB_SET(payload, '{${arr[i]}}', ${typeof payload[j]['$set'][arr[i]] === 'string' ? `'"${payload[j]['$set'][arr[i]]}"')` : payload[j]['$set'][arr[i]]}`;
  ...
  async function updateJson(query, payload, table) {
    logger.log('updateJson ' + util.inspect(payload), 1); //[{id, $set:{field1:37, field2: 45}}]
    const json_field = query.json_field;

    for (j = 0; j < payload.length; j++) {
      const arr = Object.keys(payload[j]['$set']);
      const values = arr.map(i => `'$.${i}', ${getJsonItemVal(payload[j], i)}`).join(', ');
      const sql = `UPDATE ${table} SET payload = JSONB_SET(payload, ${values}) WHERE ID = ${payload[j].id}`;
      logger.log('updateJson ' + sql);
      logger.log(sql);
      try {
  ...
  // NEW
  function getJsonItemVal(pItem, i) {
    if (typeof pItem['$set'][i] === 'string') return `'${pItem['$set'][i]}'`;
    if (Array.isArray(pItem['$set'][i])) return ` json_array (${pItem['$set'][i]})`;
    return pItem['$set'][i];
  }
...
function getColumns(tableName) {
  switch (tableName) {
    ...
    case 'formulas':
      return ['id', 'fid', 'rid', 'title', 'active', 'ts', 'description', 'comments', 'jhead', 'jrows'];
    default:
      return ['ts', 'id', 'val'];
  }
}